...
### 代号5
- **源文件目录**:`data/input/5/`,平铺放置;按 stem 小写前缀识别:`admin-` / `ibfpay-`(订单明细,手续费置0)/ `ibf平台`(资金流水账,两行合并)/ `superpay-` / `wangupay-`|`wangguypay-` / `usdt奖品发放信息`|`binance-`|`merged-`(Binance 聚合型,读 macro 合并后文件、日期取自逐行 `date` 列)
